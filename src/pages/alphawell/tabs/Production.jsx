...
export default function Production() {
  ...
  const peakOilIdx = productionData.reduce(
    (maxIdx, d, i) => (d.oil > productionData[maxIdx].oil ? i : maxIdx),
    0
  );
  const peakGasIdx = productionData.reduce(
    (maxIdx, d, i) => (d.gas > productionData[maxIdx].gas ? i : maxIdx),
    0
  );
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <NumberTile
            ...
            value={`Month ${peakOilIdx + 1}`}
            sub={`${productionData[peakOilIdx].oil.toFixed(0)} bbl/mo`}
            color="bg-orange-50"
            t="text-orange-800"
            v="text-orange-900"
          />
          <NumberTile
            ...
            value={`Month ${peakGasIdx + 1}`}
            sub={`${productionData[peakGasIdx].gas.toFixed(0)} mcf/mo`}
            color="bg-purple-50"
            t="text-purple-800"
            v="text-purple-900"
          />
          <NumberTile
            ...
            value={`${(
              productionData[productionData.length - 1].cumulativeWater / 1000
            ).toFixed(0)}K bbl`}

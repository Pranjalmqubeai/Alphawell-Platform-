...
export default function Neighborhood() {
 const { neighborWells, wellParams } = useAlphaWell();
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="px-3 py-2 cursor-pointer  rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700"
          >
        ...
        <div className="bg-gradient-to-br from-blue-50 to-emerald-50 rounded-xl p-8 mb-6 border-2 border-blue-200">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              Target Well: {wellParams.latitude.toFixed(4)},{" "}
                  ...
                  <td className="px-4 py-3 text-sm">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        well.carbonIntensity < 43
                          ? "bg-green-100 text-green-800"
                          : well.carbonIntensity < 50
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h3 className="text-lg font-bold text-gray-900 mb-4">
            EUR Distribution Comparison
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={neighborWells}>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            title="Avg EUR"
            value={`${(avg(neighborWells, "eur") / 1000).toFixed(0)}K bbl`}
            color="bg-blue-50"
          ...
          <StatCard
            title="Avg NPV"
            value={`$${avg(neighborWells, "npv").toFixed(1)}M`}
            color="bg-green-50"
            tcolor="text-green-800"
            vcolor="text-green-900"
          />
          <StatCard
            title="Avg Carbon Intensity"
            value={`${avg(neighborWells, "carbonIntensity").toFixed(0)} g/BOE`}
            color="bg-teal-50"
            tcolor="text-teal-800"
            vcolor="text-teal-900"
          />
          <StatCard
            title="Your Well Rank"
            value="Top 33%"
            color="bg-purple-50"
